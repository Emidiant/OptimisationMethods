...
def find_min_on_line(x1, x2, eps):
    ...
    if h != 0:
        ...
        if xk1 - h / 2 > xk2 - h:
            ...
            right = xk2
            left = xk1 - h / 2
        # print('Finding the minimum function on the line',
#              '[', left, ';', right, ']')
        borders = []
        borders += [left]
        borders += [right]
...
def main():
    ...
    while m.fabs(f1 - f0) > eps or counter == 0:
        print(m.sqrt(der1 ** 2 + der2 ** 2))
        print(norm([der1, der2]))
        print(der1, der2)
        if m.sqrt(der1 ** 2 + der2 ** 2) < 0.05:
            break
        if well_counter > 10:
            well_counter = 0
            x1 = 0
